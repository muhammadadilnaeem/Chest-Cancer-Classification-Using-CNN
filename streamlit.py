...
# normal_images = ["sample_images/normal/1.png", "sample_images/normal/2.png", "sample_images/normal/3.png", "sample_images/normal/4.png", "sample_images/normal/5.png"]
# cancer_images = ["sample_images/cancer/6.png", "sample_images/cancer/7.png", "sample_images/cancer/8.png", "sample_images/cancer/9.png", "sample_images/cancer/10.png"]

# # Heading
# st.title("Select Sample Images for Prediction 🖼️")

# # Display Normal Images
# with st.expander("📂 Normal Images 🩺"):
#     with st.container():
#         st.subheader("📂 Normal Images 🩺")
#         cols = st.columns(len(normal_images))
#         for i, img_path in enumerate(normal_images):
#             with cols[i]:
#                 if st.button(f"🖼️ Select Image {i+1}", key=f"normal_{i}"):
#                     selected_image_path = img_path
#                     selected_category = "Normal"
#                 st.image(img_path, caption=f"🌟 Normal Image {i+1}", use_container_width=True)


# # Display Adenocarcinoma Cancer Images
# with st.expander("📂 Adenocarcinoma Cancer Images 🧬"):
#     with st.container():
#         st.subheader("📂 Adenocarcinoma Cancer Images 🧬")
#         cols = st.columns(len(cancer_images))
#         for i, img_path in enumerate(cancer_images):
#             with cols[i]:
#                 if st.button(f"🖼️ Select Image {i+1}", key=f"cancer_{i}"):
#                     selected_image_path = img_path
#                     selected_category = "Cancer"
#                 st.image(img_path, caption=f"⚠️ Cancer Image {i+1}", use_container_width=True)
...
#     st.markdown(f"### 📸 Selected Sample Image from {selected_category} Category")
#     selected_image = Image.open(selected_image_path)
#     st.markdown('<div class="card">', unsafe_allow_html=True)
    
#     col1, col2 = st.columns(2)
#     with col1:
#         st.image(selected_image, caption="🔍 Selected Sample X-Ray Image", use_container_width=True)
#     with col2:
#         st.markdown("### Image Details")
#         st.markdown(f"**File Name:** {os.path.basename(selected_image_path)}")  # Fix applied here
#         st.markdown(f"**Image Size:** {selected_image.size}")
#         st.markdown(f"**File Format:** {selected_image.format}")
#     st.markdown('</div>', unsafe_allow_html=True)


#     if st.button("🔬 Analyze Selected Image", key="analyze_sample_button"):
...
#             prediction_2, confidence_2 = pipeline.predict(selected_image)
...
#                     f"""
#                     <div class="result-box normal">
#                         ✅ Result: {prediction_2}<br>
#                         Confidence: {confidence_2:.2f}%<br>
#                         No signs of cancer detected
#                     </div>
#                     """,
#                     unsafe_allow_html=True
        ...
        for i, img_path in enumerate(normal_images):
            with cols[i]:
                if st.button(f"🖼️ Select Image {i+1}", key=f"normal_{i}"):
                    ...
                    st.session_state.selected_category = "Normal"
                st.image(img_path, caption=f"🌟 Normal Image {i+1}", use_container_width=True)
    ...
    with st.container():
        ...
        for i, img_path in enumerate(cancer_images):
            with cols[i]:
                if st.button(f"🖼️ Select Image {i+1}", key=f"cancer_{i}"):
                    st.session_state.selected_image_path = img_path
                    st.session_state.selected_category = "Cancer"
                st.image(img_path, caption=f"⚠️ Cancer Image {i+1}", use_container_width=True)
...
if st.session_state.selected_image_path:
    ...
    with col2:
        st.markdown("### Image Details")
        st.markdown(f"**File Name:** {os.path.basename(selected_image_path)}")
        st.markdown(f"**Image Size:** {selected_image.size}")
        st.markdown(f"**File Format:** {selected_image.format}")
    ...
    if st.button("🔬 Analyze Selected Image", key="analyze_sample_button"):
        with st.spinner("🛠️ Processing image..."):
            ...
            # Get prediction
            prediction, confidence = pipeline.predict(selected_image)
            ...
            # Reset session state after displaying results
            st.session_state.selected_image_path = None
            st.session_state.selected_category = None
